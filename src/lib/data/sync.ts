...
async function catchUp(
  ...
  const loaded = await getCloudSource().loadKitchenOrder(orderId);
  let current: OrderStatus = loaded.data?.status ?? "pending";

  if (current === target || RANK[current] >= RANK[target]) {
    return null;
  }
...
async function pushOne(record: LocalOrder, clientId: string): Promise<boolean> {
  ...
  if (orderId === null) {
    ...
    const result = await getCloudSource().submitOrder({
      ...
      ticketDate: record.order.ticket_date,
